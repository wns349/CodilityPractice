...
public class Fish {
	 ...
	 * other (living) fish between them, they will eventually meet each other.
	 ...
	 * B[P] = 1 and B[Q] = 0, and there are no living fish between them. After
	 * they meet:
	 * 
	 * If A[P] > A[Q] then P eats Q, and P will still be flowing downstream, If
	 * A[Q] > A[P] then Q eats P, and Q will still be flowing upstream. We
	 ...
	 * 
	 * A[0] = 4 B[0] = 0 A[1] = 3 B[1] = 1 A[2] = 2 B[2] = 0 A[3] = 1 B[3] = 0
	 * A[4] = 5 B[4] = 0 Initially all the fish are alive and all except fish
	 * number 1 are moving upstream. Fish number 1 meets fish number 2 and eats
	 * it, then it meets fish number 3 and eats it too. Finally, it meets fish
	 ...
	 * elements of A are all distinct. Complexity:
	 * 
	...
	public int solution(int[] A, int[] B) {
		FishInfo[] river = new FishInfo[A.length];
		int riverSize = 0;
		river[riverSize++] = new FishInfo(A[0], B[0]);

		for (int i = 1; i < A.length;) {
			boolean readyForNewFish = true;
			FishInfo topFish = river[riverSize - 1];
			FishInfo newFish = new FishInfo(A[i], B[i]);

			if (newFish.dir == topFish.dir) {
				// same direction, would not be killed
				river[riverSize++] = newFish;
			} else if (newFish.dir == 1 && topFish.dir == 0) {
				// Different direction, but different stream. Would not meet
				river[riverSize++] = newFish;
			} else {
				// Different direction, check who wins
				if (newFish.size > topFish.size) {
					riverSize--; // TopFish is dead
					if (riverSize <= 0) {
						// No more fish, add newFish
						river[riverSize++] = newFish;
					} else {
						// Top fish dead, need to check if new fish wins again
						// So, do not increment loop
						readyForNewFish = false;
					}
				}
			}
