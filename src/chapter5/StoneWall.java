...
public class StoneWall {
	 ...
	 * 
	 * H[0] = 8 H[1] = 8 H[2] = 5 H[3] = 7 H[4] = 9 H[5] = 8 H[6] = 7 H[7] = 4
	 * H[8] = 8 the function should return 7. The figure shows one possible
	 ...
	 * an integer within the range [1..1,000,000,000]. Complexity:
	 * 
	...
	public int solution(int[] H) {
		...
		// Initially, push first item to stack
		stack[size++] = H[0];

		for (int i = 1; i < H.length; i++) {
			while ((size - 1) >= 0 && stack[size - 1] > H[i]) {
				// Pop
				size--;
				ans++;
			}
		...
		// Finally, pop everything
		ans += size;
		return ans;
	}

	public static void main(String[] args) {
		StoneWall o = new StoneWall();
		int[] H = { 8, 8, 5, 7, 9, 8, 7, 4, 8 };
		System.out.println(o.solution(H));
	}
}
