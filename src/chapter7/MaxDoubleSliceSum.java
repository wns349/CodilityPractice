...
public class MaxDoubleSliceSum {
	 ...
	 * 
	 * double slice (0, 3, 6), sum is 2 + 6 + 4 + 5 = 17, double slice (0, 3,
	 * 7), sum is 2 + 6 + 4 + 5 − 1 = 16, double slice (3, 4, 5), sum is 0. The
	 * goal is to find the maximal sum of any double slice.
	 * 
	 * Write a function:
	 * 
	...
	public int solution(int[] A) {
		...
		for (int i = 1; i < A.length - 1; i++) {
			front[i] = front[i - 1] + A[i] > 0 ? front[i - 1] + A[i] : 0;
		}

		for (int i = A.length - 2; i > 0; i--) {
			end[i] = end[i + 1] + A[i] > 0 ? end[i + 1] + A[i] : 0;
		}

		int max = 0;
		for (int i = 0; i < A.length - 2; i++) {
			max = front[i] + end[i + 2] > max ? front[i] + end[i + 2] : max;
		}
