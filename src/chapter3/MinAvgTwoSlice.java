...
public class MinAvgTwoSlice {
	 ...
	 * 
	 * A[0] = 4 A[1] = 2 A[2] = 2 A[3] = 5 A[4] = 1 A[5] = 5 A[6] = 8 contains
	 * the following example slices:
	 * 
	 * slice (1, 2), whose average is (2 + 2) / 2 = 2; slice (3, 4), whose
	 * average is (5 + 1) / 2 = 3; slice (1, 4), whose average is (2 + 2 + 5 +
	 * 1) / 4 = 2.5. The goal is to find the starting position of a slice whose
	 * average is minimal.
	 * 
	 * Write a function:
	 * 
	 ...
	 * an integer within the range [−10,000..10,000]. Complexity:
	 * 
	...
	public int solution(int[] A) {
		float minAverageValue = Float.MAX_VALUE;
		int minAveragePosition = 0;

		for (int i = 0; i < A.length - 2; i++) {
			// 2 elements
			float average = (float) (A[i] + A[i + 1]) / 2.0f;
			if (average < minAverageValue) {
				minAverageValue = average;
				minAveragePosition = i;
			}

			// 3 elements
			average = (float) (A[i] + A[i + 1] + A[i + 2]) / 3.0f;
			if (average < minAverageValue) {
				minAverageValue = average;
				minAveragePosition = i;
			}
		...
		// Check last 2
		if (A.length >= 2) {
			float average = (float) (A[A.length - 2] + A[A.length - 2 + 1]) / 2.0f;
			if (average < minAverageValue) {
				minAverageValue = average;
		...
		// Check last 3
		if (A.length >= 3) {
			float average = (float) (A[A.length - 3] + A[A.length - 3 + 1] + A[A.length - 3 + 2]) / 3.0f;
			if (average < minAverageValue) {
				minAverageValue = average;
				minAveragePosition = A.length - 3;
			}
		}
