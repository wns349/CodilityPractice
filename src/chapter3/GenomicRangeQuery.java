...
public class GenomicRangeQuery {
	 ...
	 * respectively. You are going to answer several queries of the form: What
	 * is the minimal impact factor of nucleotides contained in a particular
	 ...
	 * (0 ≤ K < M) requires you to find the minimal impact factor of nucleotides
	 * contained in the DNA sequence between positions P[K] and Q[K]
	 * (inclusive).
	 * 
	 ...
	 * is 2. The part between positions 5 and 5 contains a single nucleotide T,
	 * whose impact factor is 4, so the answer is 4. The part between positions
	 * 0 and 6 (the whole string) contains all nucleotides, in particular
	 * nucleotide A whose impact factor is 1, so the answer is 1. Write a
	 * function:
	 * 
	 ...
	 * returns an array consisting of M integers specifying the consecutive
	 ...
	 * programming language). For example, given the string S = CAGCCTA and
	 ...
	 * range [1..50,000]; each element of arrays P, Q is an integer within the
	...
	public int[] solution(String S, int[] P, int[] Q) {
		...
		for (int i = 0; i < P.length; i++) {
			for (int j = 0; j < 4; j++) {
				int total = countTotal(counter, j, P[i], Q[i]);
				if (total > 0) {
					ans[i] = j + 1;
					break;
				}
	...
	public int countTotal(int[][] A, int type, int left, int right) {
		return A[type][right + 1] - A[type][left];
	}
	...
	public static void main(String[] args) {
		...
		int[] P = { 2, 5, 0 };
		int[] Q = { 4, 5, 6 };
		System.out.println(Arrays.toString(grq.solution(S, P, Q)));
	}
}
